...
export default function ResetPasswordPage() {
  ...
  const isFormValid = hasMinLen && hasUpper && hasNumber && hasSpecial && password === confirm;

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!isFormValid) {
      triggerToast("Por favor, preencha todos os requisitos de segurança exigidos.");
      return;
    }

    setLoading(true);
    triggerToast("Processando atualização de senha corporativa...");
    setTimeout(() => {
      setLoading(false);
      setSuccess(true);
      triggerToast("Sua senha foi alterada com sucesso! Redirecionando...");
      setTimeout(() => {
        router.push('/login');
      }, 2500);
    }, 1500);
        ...
        <div className="hidden lg:flex flex-col text-left space-y-7 animate-in fade-in slide-in-from-left-6 duration-700">
          ...
          <div className="space-y-4">
            <h2 className="text-[28px] xl:text-[34px] font-black tracking-tight text-[#1E1538] leading-tight">
              Redefina sua senha com<br />padrões máximos de segurança.
            </h2>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 pt-3">
            {[
              { icon: Shield, title: 'Criptografia forte', desc: 'Senhas criptografadas via Argon2id' },
              { icon: Lock, title: 'Segurança absoluta', desc: 'Exigência de complexidade robusta' },
              { icon: FileText, title: 'Sessões revogadas', desc: 'Invalidamento de acessos antigos' }
            ].map((pilar) => {
        ...
        <div className="flex flex-col space-y-4 items-center justify-center animate-in fade-in slide-in-from-right-6 duration-700">
          ...
          <div className="bg-white border border-[#E8DDFD]/90 rounded-[28px] p-7 xl:p-10 shadow-2xl shadow-purple-950/5 w-full max-w-[460px] text-left space-y-6 relative overflow-hidden">
            ...
            <div className="space-y-1.5">
              <h3 className="text-[22px] font-black tracking-tight text-[#1E1538]">Nova senha</h3>
              <p className="text-slate-450 font-bold text-xs leading-relaxed mt-1">
                Escolha uma nova senha que atenda aos requisitos mínimos de segurança da política de dados da Basileia Pay.
              </p>
            </div>
            ...
            {success ? (
              <div className="bg-emerald-50/75 border border-emerald-100 text-emerald-850 rounded-2xl p-5 text-xs font-semibold leading-relaxed space-y-2">
                ...
                <p>
                  Sua senha foi alterada com sucesso no sistema. Você está sendo redirecionado para a página de login agora mesmo.
                </p>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-1.5">
                  <label className="text-[11px] font-black uppercase text-slate-400 tracking-wider">Confirmar nova senha</label>
                  <input
                    type={showPassword ? "text" : "password"}
                    required
                    ...
                    className="w-full h-11.5 px-4 rounded-xl border border-slate-200 focus:border-brand focus:ring-1 focus:ring-brand outline-none text-xs font-bold text-slate-800 transition-all placeholder:text-slate-350 bg-slate-50/50 focus:bg-white"
                  />
                </div>

                {/* Requirements check list */}
                <div className="bg-[#FAF8FF] border border-[#E8DDFD]/65 rounded-xl p-3.5 space-y-2 text-[11px]">
                  <div className="text-[9px] font-black uppercase text-slate-400 tracking-wider pb-0.5 border-b border-[#E8DDFD]/35">
                    Critérios de senha
                  </div>
                  
                  <div className="grid grid-cols-2 gap-x-2 gap-y-1.5 font-bold">
                  ...
                  {password && confirm && (
                    <div className="pt-1.5 border-t border-[#E8DDFD]/35 flex items-center gap-1.5 font-black text-[9px] uppercase tracking-wider">
                      {password === confirm ? (
                        <>
                          <Check className="w-3.5 h-3.5 text-emerald-500 shrink-0" />
                          <span className="text-emerald-700">As senhas coincidem</span>
                        </>
                      ) : (
                        <>
            ...
            <div className="text-center">
              <Link
                href="/login"
                className="inline-flex items-center gap-1.5 text-xs font-black text-slate-400 hover:text-brand transition-colors uppercase tracking-wider"
              >
