...
<body>
    <div class="container" x-data="{ 
        ...
        <div class="left-panel">
            ...
            <h1 class="plan-title">{{ $transaction->description ?? 'Plano Profissional' }}</h1>
            <div class="plan-price" x-text="formatCurrency({{ $transaction->amount }})"></div>
            <p class="plan-billing">{{ $transaction->customer_name ?? 'Basileia Global' }}</p>
        ...
        <div class="right-panel">
            ...
            <div x-show="method === 'credit_card'" x-cloak style="flex: 1; display: flex; flex-direction: column;">
                <form method="POST" action="{{ route('checkout.process', $transaction->uuid) }}" style="flex: 1; display: flex; flex-direction: column;">
                    @csrf
                    ...
                    <button type="submit" class="cta-button">
                        🔒 <span x-text="t('pay')"></span> <span x-text="formatCurrency({{ $transaction->amount }})"></span>
                    </button>
                </form>
            </div>
            ...
            <div x-show="method === 'pix'" x-cloak class="pix-content" style="flex: 1;">
                @if($transaction->pix_qr_code ?? false)
                    <div class="pix-qrcode">
                        <img src="data:image/png;base64,{{ $transaction->pix_qr_code_image ?? '' }}" alt="QR Code PIX" width="180" height="180">
                    </div>
                    <p style="font-size: 13px; color: #666; margin-bottom: 12px;" x-text="t('scanQr')"></p>
                    <div class="pix-code">{{ $transaction->pix_copy_paste ?? '' }}</div>
                    <button onclick="navigator.clipboard.writeText('{{ $transaction->pix_copy_paste ?? '' }}')" class="pix-copy" x-text="t('copyCode')"></button>
                @else
                    <form method="POST" action="{{ route('checkout.process', $transaction->uuid) }}">
                        @csrf
                        <input type="hidden" name="payment_method" value="pix">
                        <button type="submit" class="cta-button" x-text="t('pay') + ' ' + formatCurrency({{ $transaction->amount }})"></button>
                    </form>
                @endif
            </div>
            ...
            <div x-show="method === 'boleto'" x-cloak class="boleto-content" style="flex: 1;">
                @if($transaction->boleto_url ?? false)
                    <a href="{{ $transaction->boleto_url }}" target="_blank" class="cta-button" style="display: block; text-decoration: none;">
                        📄 <span x-text="t('viewBoleto')"></span>
                    </a>
                    @if($transaction->boleto_expiration)
                        <p style="font-size: 13px; color: #666; margin-top: 16px;">
                            <span x-text="t('dueDate')"></span>: {{ $transaction->boleto_expiration }}
                        </p>
                    @endif
                @else
                    <form method="POST" action="{{ route('checkout.process', $transaction->uuid) }}">
